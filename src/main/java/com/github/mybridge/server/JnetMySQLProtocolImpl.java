...
public class JnetMySQLProtocolImpl implements MySQLProtocol {
    ...
    private MySQLHandler                  handler = new DefaultMySQLHandler();
            ...
            case READ_AUTH:
                ...
                try {
                    if (MySQLCommand.index2Charset.containsKey((int) authPacket.charsetNum)) {
                        handler.setCharset(MySQLCommand.index2Charset.get((int) authPacket.charsetNum));
                    }
                    if (authPacket.databaseName.length() > 0) {
                        String dbname = authPacket.databaseName.substring(0, authPacket.databaseName.length() - 1);
